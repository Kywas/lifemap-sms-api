...
app.post("/api/auth/send-code", async (req, res) => {
  ...
  if (!gate.ok) {
    return res.status(429).json({
      ok: false,
      error: `Подождите ${gate.waitSec} сек перед повторной отправкой`,
      retryAfterSec: gate.waitSec,
    });
  }

  const code = generateOtpCode();
  const message = `Life Map: код входа ${code}. Никому не сообщайте его.`;

  try {
    await sendSmsNow(phone, message);
    createOtp(phone, code);
    if (!IS_PROD) console.log("[auth] dev code for", phone, code);
    res.json({ ok: true, phone, expiresInSec: 300 });
  } catch (err) {
    console.error("[auth/send-code]", phone, err.message);
    res.status(502).json({ ok: false, error: err.message || "Не удалось отправить код" });
  }
});
...
app.post("/api/auth/profile", (req, res) => {
  ...
  const name = String(req.body?.name || "").trim().slice(0, 40);
  updateUserName(user.phone, name);
  res.json({
    ok: true,
    phone: user.phone,
    phoneDisplay: formatPhoneDisplay(user.phone),
...
app.post("/api/auth/logout", (req, res) => {
  if (!checkAuth(req, res)) return;
  const user = getUserFromRequest(req);
  if (user?.token) deleteSession(user.token);
  res.json({ ok: true });
});
...
app.post("/api/sms/schedule", async (req, res) => {
  ...
  const user = getUserFromRequest(req);
  if (!user) {
    return res.status(401).json({ ok: false, error: "Войдите по номеру телефона" });
  }
  ...
  const message = applySenderPrefix(rawMessage, user.phone, user.name);
...
app.post("/api/sms/update", (req, res) => {
  ...
  const rawMessage = String(req.body?.message || "").trim() || buildTimerSmsMessage(geo);
  const message = applySenderPrefix(rawMessage, user.phone, user.name);
...
app.post("/api/sms/test", async (req, res) => {
  ...
  try {
    const smsId = await sendSmsNow(phone, message.slice(0, 320));
